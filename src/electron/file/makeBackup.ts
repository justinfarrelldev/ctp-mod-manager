...
import { DEFAULT_BACKUPS_FOLDER, DEFAULT_BACKUPS_FOLDER_NAME } from '../constants';
...
export const selectivelyAddBackupsFolder = async (): Promise<void> => {
  ...
  if (stats) {
    if (!stats.isDirectory()) await createAppDataFolder(DEFAULT_BACKUPS_FOLDER_NAME);
  }
};
...
export const makeBackup = async (installDir: string): Promise<void> => {
  ...
  try {
    ...
    const installDirAsFileName = installDir
      ...
      .replaceAll('(', '')
      .replaceAll(')', '');
    console.log('writing zip to: ', `${DEFAULT_BACKUPS_FOLDER}\\${installDirAsFileName}.zip`);
    zip.writeZip(`${DEFAULT_BACKUPS_FOLDER}\\${installDirAsFileName}.zip`);
    console.log('wrote zip there...');
  } catch (err) {
    console.error(`An error occurred while creating a zip file for a backup: ${err}`);
  }
};
